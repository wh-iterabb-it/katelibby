...
const command = require('../../commands/fortune');
...
describe('Command Test', () => {
  describe('fortune command', () => {
    it('should return a random but valid fortune', () => {
      const expected = [
      ...
      const args = '';
      let callback = {};
      callback.say = (to, message) => {
        const check = expectedSet.has(message);
        expect(check).to.equal(true);
      };
      const resp = command(callback, target, from, args);
    });
  });
});
